...
End-to-end two-stage dose training on full volumes with optional DistributedDataParallel (DDP).

Stage 1: UNet3D (CT + 9 structure masks) -> coarse dose prediction
Stage 2: ResUNet3D (CT + 9 masks + coarse prediction + 4 strip masks) -> final dose prediction

The script logs training/validation metrics to TensorBoard and saves checkpoints.
